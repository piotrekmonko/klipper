...
class WebDwc2:
        ...
        if "/macros" in web_.get_argument('dir').replace("0:", ""):
            for macro_ in self.klipper_macros:
                repl_['files'].append({
                    ...
                    "name": macro_,
                    "size": 1,
                    "date": datetime.datetime.fromtimestamp(os.stat(self.klipper_config).st_mtime).strftime(
                        "%Y-%m-%dT%H:%M:%S")
                })
    ...
    def rr_gcode(self, web_):
        ...
        midprint_allow = ['DUMP_TMC', 'G10', 'GET_POSITION', 'HELP', 'M0', 'M140', 'M24', 'M25', 'M104', 'M106', 'M107',
                          'M112', 'M114', 'M115', 'M140', 'M204', 'M220', 'M221', 'M290', 'M292',
                          'QUERY_FILAMENT_SENSOR', 'SET_TMC_CURRENT', 'SET_PIN',
    ...
    def rr_status_3(self):
        ...
        self.status_3.update({
            ...
            "firstLayerDuration": self.print_data.get('firstlayer_dur', 1) if self.print_data.get('firstlayer_dur',
                                                                                                  0) > 0 else self.print_data.get(
                'curr_layer_dur', 1),
    ...
    def update_printdata(self, eventtime):
        ...
        if self.print_data['curr_layer_start'] == 0 \
            ...
            if self.print_data['last_switch_z'] != gcode_stats['last_zpos'] and self.print_data['filament_used'] > 50 \
                    and (min(self.print_data.get('last_zposes', [2])) + 1) / (
                    max(self.print_data.get('last_zposes', [1])) + 1) == 1:
